...
import { VoiceRecorder, type NoteAttachment } from "./voice-recorder";

export type { NoteAttachment } from "./voice-recorder";
...
interface MediaUploaderProps {
  ...
  compact?: boolean;
  /** Mostrar solo el grabador de voz (sin el botón de adjuntar archivos). */
  audioOnly?: boolean;
  /** Transcribir las notas de voz vía Groq y emitir el texto por onTranscription. */
  transcribe?: boolean;
  ...
  async function handleFiles(files: FileList | null) {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        next.push(body.attachment as NoteAttachment);
      }
      onChange(next);
    } finally {
      setUploading(false);
    ...
    <div className="group relative flex flex-col gap-1 overflow-hidden rounded-xl border border-border bg-bg/60 p-1.5">
      ...
      {kind === "audio" && (
        <div className="flex aspect-square w-full flex-col items-center justify-center gap-1.5 rounded-lg bg-surface-elevated px-2 py-3 text-center">
          {url ? <audio src={url} controls className="w-full" /> : <Skeleton label="audio" />}
        </div>
      )}
      {kind === "file" && (
        <a
