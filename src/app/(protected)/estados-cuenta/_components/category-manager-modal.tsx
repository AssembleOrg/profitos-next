          ...
          <Dialog.Portal forceMount>
            ...
            <Dialog.Content asChild>
              <motion.div
                ...
                className="fixed left-1/2 top-1/2 z-50 flex max-h-[92dvh] w-[min(560px,calc(100vw-1.5rem))] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl"
              >
                <header className="flex items-center justify-between gap-3 border-b border-border-olive/40 px-5 py-4">
                  <Dialog.Title className="text-base font-semibold text-text">Categorías</Dialog.Title>
                ...
                <div className="flex-1 overflow-y-auto px-5 py-5">
                  {/* Alta */}
                  <div className="mb-5 flex flex-wrap items-end gap-2 rounded-xl border border-border bg-bg/40 p-3">
                    <div className="flex-1 min-w-[140px]">
                      <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-text-muted">Nueva categoría</label>
                      <input
                        type="text"
                        value={name}
                    ...
                    <input
                      ...
                      className="h-[38px] w-12 cursor-pointer rounded-xl border border-border bg-bg"
                      aria-label="Color"
                    />
                    <button
                      type="button"
                      onClick={add}
                      disabled={busy}
                      className="rounded-xl border border-olive-bright/30 bg-olive-mid px-4 py-2 text-sm font-semibold text-bg transition-colors hover:bg-olive-vivid disabled:opacity-60"
                    >
                      Agregar
                    </button>
                  </div>
...
function CategoryColumn({
  ...
  return (
    <div>
      <p className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-accent/70">{title}</p>
      <div className="flex flex-col gap-1.5">
        {items.map((c) => (
          <div key={c.id} className="flex items-center gap-2 rounded-lg border border-border bg-bg/40 px-3 py-2">
            ...
            {c.isSystem ? (
              <span className="shrink-0 rounded-full bg-bg px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-text-faint">
                Sistema
              </span>
            ) : (
              <div className="flex shrink-0 items-center gap-1">
                <button
                  ...
                  className="flex h-7 w-7 items-center justify-center rounded-md text-text-faint transition-colors hover:bg-surface hover:text-text"
                >
