...
import { ObjetivosClient } from "./_components/objetivos-client";
import { serializeCard } from "./_components/serialize";
...
export default async function ObjetivosPage({ searchParams }: Readonly<Props>) {
  const user = await getCurrentUser();
  if (!user) redirect("/login");
  if (user.role !== "admin") redirect("/mis-objetivos");
  ...
  const [cards, total, users] = await Promise.all([
