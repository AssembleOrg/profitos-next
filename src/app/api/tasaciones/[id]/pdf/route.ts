...
import { PDFDocument, PDFImage, StandardFonts, rgb, degrees, type PDFPage } from "pdf-lib";
...
function drawBranding(
  ...
  page.drawLine({
    start: { x: brandX, y: brandY - 2 },
    end: { x: W - 4, y: brandY - 2 },
    thickness: 0.5,
    color: rgb(0.3, 0.3, 0.3),
  });

  const colText = "Martillera – COL. 1011";
  const colW = fontR.widthOfTextAtSize(colText, 9.5);
...
function drawBottomLeftCorner(page: PDFPage) {
  const dark = rgb(0.17, 0.17, 0.17);
  const mid = rgb(0.52, 0.52, 0.52);
  const white = rgb(1, 1, 1);

  const r = 165;

  drawSolidDiamond(page, 8, 8, r, dark);
  drawSolidDiamond(page, 24, 24, r, white);
  drawSolidDiamond(page, 46, 46, r, mid);
  drawSolidDiamond(page, 70, 70, r, white);
  drawSolidDiamond(page, 96, 96, r, dark);

  drawSolidDiamond(page, 78, 78, 58, mid);
  drawSolidDiamond(page, 78, 78, 40, white);
  drawSolidDiamond(page, 78, 78, 24, dark);
}

function drawTopRightCorner(page: PDFPage) {
...
function drawCornerDecorations(page: PDFPage) {
  drawBottomLeftCorner(page);
  drawTopRightCorner(page);
}
...
export async function GET(
  ...
  try {
    ...
    const subX = (W - subW) / 2;
    const subY = titleY - 42; // más pegado al título
    const lineGap = 18;

    p1.drawLine({
      start: { x: titleX - 10, y: subY + 10 },
      end: { x: subX - lineGap, y: subY + 10 },
      thickness: 1.2,
    ...
    p1.drawLine({
      start: { x: subX + subW + lineGap, y: subY + 10 },
      end: { x: titleX + titleW + 10, y: subY + 10 },
      thickness: 1.2,
      color: rgb(0.15, 0.15, 0.15),
    });

    const addrSize = 25; // +6 px aprox respecto al anterior 19
    const addrW = fontR.widthOfTextAtSize(direccion, addrSize);
    p1.drawText(direccion, {
      x: (W - addrW) / 2,
      y: subY - 50, // 6 px más arriba aprox
      size: addrSize,
      font: fontR,
    ...
    drawCornerDecorations(p2);

    const leftX = 16;

    p2.drawText("TASACION", {
      x: leftX,
      y: H - 42,
      size: 36,
      font: fontB,
    ...
    p2.drawText("A quien corresponda:", {
      x: leftX,
      y: y2,
      size: 12,
      font: fontR,
    ...
    for (const b of bullets) {
      ...
      const labelW = fontB.widthOfTextAtSize(label, 15);
    ...
    const tablas = Array.isArray(tasacion.tablas)
      ? (tasacion.tablas as Array<{ titulo: string; filas: Array<{ unidad: string; valor: string; observaciones: string }> }>)
      : [];
    ...
    if (tablas.length > 0) {
      ...
      for (const tabla of tablas) {
        ...
        for (let ci = 0; ci < 3; ci++) {
          ...
          if (ci < 2) {
            pg.drawLine({
              start: { x: cx + colW[ci], y: ty },
              end: { x: cx + colW[ci], y: ty - headerH },
              thickness: 0.5,
              color: rgb(0.12, 0.12, 0.12),
            });
          }
        ...
        for (const fila of tabla.filas) {
          ...
          for (let ci = 0; ci < 3; ci++) {
            ...
            if (ci < 2) {
              pg.drawLine({
                start: { x: rx + colW[ci], y: ty },
                end: { x: rx + colW[ci], y: ty - rowH },
                thickness: 0.5,
                color: rgb(0.6, 0.6, 0.6),
              });
            }
