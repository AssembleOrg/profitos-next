...
import { VoiceRecorder, type RentalAttachment } from "./voice-recorder";

interface MediaUploaderProps {
  attachments: RentalAttachment[];
  onChange: (next: RentalAttachment[]) => void;
  ...
  async function handleFiles(files: FileList | null) {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        next.push(body.attachment as RentalAttachment);
      }
      onChange(next);
    } finally {
      setUploading(false);
    ...
    <div className="flex flex-col gap-2">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <input
          ...
          className="hidden"
        />
        <VoiceRecorder onUploaded={(att) => onChange([...attachments, att])} disabled={uploading} />
      </div>
