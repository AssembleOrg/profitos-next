...
interface VoiceRecorderProps {
  ...
  disabled?: boolean;
  /** Si está activo, además de subir el audio lo transcribe vía Groq. */
  transcribe?: boolean;
  onTranscription?: (text: string) => void;
}

type RecorderState = "idle" | "recording" | "review" | "uploading" | "transcribing";
...
export function VoiceRecorder({ onUploaded, disabled, transcribe, onTranscription }: Readonly<VoiceRecorderProps>) {
  ...
  async function upload() {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await fetch("/api/notas/upload", { method: "POST", body: fd });
      const body = await res.json();
      if (!res.ok) throw new Error(body?.error ?? "Error al subir");
      onUploaded(body.attachment as NoteAttachment);
      toast.success("Nota de voz lista");

      if (transcribe && onTranscription) {
        setState("transcribing");
        try {
          const tf = new FormData();
          tf.append("file", file);
          const tr = await fetch("/api/notas/transcribe", { method: "POST", body: tf });
          const tb = await tr.json();
          if (tr.ok && typeof tb.text === "string" && tb.text.trim()) {
            onTranscription(tb.text.trim());
            toast.success("Audio transcripto");
          } else {
            toast.error(tb?.error ?? "No se pudo transcribir el audio");
          }
        } catch {
    ...
    <div className="flex items-center gap-2">
      <AnimatePresence mode="wait">
        ...
        {state === "review" && previewUrl && (
          <motion.div key="review" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-1 items-center gap-2">
            ...
            <button
              type="button"
              onClick={upload}
              className="rounded-xl border border-olive-bright/30 bg-olive-mid px-3 py-2 text-xs font-semibold text-bg hover:bg-olive-vivid"
            >
              Adjuntar
            </button>
            <button
        ...
        {(state === "uploading" || state === "transcribing") && (
          <motion.div key="up" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="inline-flex items-center gap-2 rounded-xl border border-border bg-bg px-3 py-2 text-xs text-text-muted">
            <span className="h-3 w-3 animate-spin rounded-full border-2 border-olive-bright border-t-transparent" />
            {state === "transcribing" ? "Transcribiendo…" : "Subiendo…"}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
